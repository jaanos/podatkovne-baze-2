...
Ta orodja smo predstavili le informativno in jih ne bomo nujno potrebovali za nadaljne delo pri predmetu!
...
Ime je pogosto kar "venv".\
Aktivacija okolja je odvisna od operacijskega sistema:\
Windows: `"ime_datoteteke"/Scripts/Activate`\
Linux: `source "ime datoteke"\bin\activate`\

Po aktivaciji se nam virtualno okolje izpiše na začetku terminala. Virtualno okolje lahko deaktiviramo z `deactivate`.

Ko smo v virtualnem okolju, lahko inštaliramo poljubne knjižnice (z npr. `pip install`) in bodo te vidne le v danem okolju.
...
Recimo, da v golem ("vanilla") Pythonovem okolju namestimo vse kar želimo (na vajah smo namestili vse dodatne knjižnice, ki jih potrebuje profesorjev orm). Nameščene verzije si shranemo v datoteko enostavno z:\
...
Ta orodja lahko ženemo iz terminala (namestimo jih lahko s `pip`), v praksi pa jih namestimo kar kot VSCode extension.
...
Pogledali smo si kako lahko teste iz predavanj napišemo s pomočjo Pythonove knjižnice `unittest`.\
`unittest` je priporočen tudi na Django-tovi spletni strani, sicer pa se dandanes pogosto uporabljajo tudi druge knjižnice za testiranje (npr. `pytest`).

V datoteki **testi_primer_unittest.py** lahko vidimo primer uporabe knjižnice `unittest`.
